...
void drawButton(uint16_t xPos, uint16_t yPos, uint8_t *label, uint32_t color, uint8_t buttonSize)
{
	...
	BSP_LCD_DisplayStringAt(xPos-8, yPos-10, label, LEFT_MODE);
}
...
uint8_t checkButton(volatile uint16_t *xCoord,volatile uint16_t *yCoord)
{
	...
	for(uint8_t i = 0; i <= 3; i++)
	{
		...
		{
			if(*xCoord > 175+(60*j) && *yCoord > 25+(60*i) && *xCoord < 175+(60*j)+BUTTON_SIZE+4 && *yCoord < 25+(60*i)+BUTTON_SIZE+4)
			{
				column = i;
				row = j;
				return getButtonDef(column,row);
			}
		}
	}
	return NULL;
...
void drawKeypad(void)
{
	...
	for(uint8_t row = 0; row <= 3; row++)
	{
		...
		for(uint8_t column = 0; column <= 4; column++)
		{
			...
			if(row != 3)
			{
				sprintf((char*)temp, "%c",getButtonDef(row,column));
...
void drawAdminDispFrame(void)
{
	...
	BSP_LCD_SetBackColor(LCD_COLOR_BLACK);
	//BSP_LCD_FillRect(0, 0, 167, BSP_LCD_GetYSize());
	BSP_LCD_DrawLine(167, 0, 167, BSP_LCD_GetYSize());
	BSP_LCD_DrawLine(168, 0, 168, BSP_LCD_GetYSize());
	//BSP_LCD_SetTextColor(LCD_COLOR_LIGHTGRAY);
	BSP_LCD_SetBackColor(LCD_COLOR_LIGHTGRAY);
	BSP_LCD_FillRect(0, 0, 165, BSP_LCD_GetYSize());
	...
	//Render Selection area
	BSP_LCD_DisplayStringAt(168, 10, (uint8_t*) "Enter Passcode", CENTER_MODE);
...
void drawPassKeypad(void)
{
	...
	for(uint8_t row = 0; row < 4; row++)
	{
		for(uint8_t column = 0; column < 3; column++)
		{
			...
			drawButton(PASS_BUTTON_XPOS(row),PASS_BUTTON_YPOS(column), (uint8_t*)temp, LCD_COLOR_BLUE,PASS_BUTTON_SIZE);
		}
	}
}
	...
	for(uint8_t i = 0; i <= 3; i++)
	{
		for(uint8_t j = 0; j <=4; j++)
		{
			if(xCoord > 175+(60*j) && yCoord > 25+(60*i) && xCoord < 175+(60*j)+BUTTON_SIZE+4 && yCoord < 25+(60*i)+BUTTON_SIZE+4)
			{
				column = i;
				row = j;
				return getPasskeyButtonDef(column,row);
			}
		}
	}
	return NULL;
